...
class DishesService {
  ...
  async createDish(name, country) {
    try {
      ...
      if (alreadyInDatabase.length == 0) {
        ...
        const createDish = await this.Dishes.create({
          Name: name,
          CountryId: CountryId,
        });
  ...
  async getAllDishes() {
    ...
    let Dishes = await this.client.query(
      "SELECT Dishes.Id, Dishes.Name as DishName, Countries.Name as Country FROM Dishes JOIN Countries on Dishes.CountryId = Countries.Id",
  ...
  async getDish(dishname) {
    // Create a JOIN RAW query to get the country and dish. Sequelize functions also can be used here
    let Dishes = await this.client.query(
      `SELECT Dishes.Id, Dishes.Name as DishName, Countries.Name as Country FROM Dishes JOIN Countries on Dishes.CountryId = Countries.Id WHERE Dishes.name = '${dishname}'`,
      { type: QueryTypes.SELECT }
    );
    return Dishes;
  }
